...
def leaderboard_by_id(request):
    ...
    return render(
        request, "leaderboard/leaderboard.html", {"users": users, "sort_by": "user_id"}
    )
...
def leaderboard_by_ability(request):
    ...
    return render(
        ...
        {"users": users, "sort_by": "ability_score"},
    )
...
def get_task_info_by_id(request):
    ...
    try:
        ...
        return Response({"error": "任务ID不存在"}, status=status.HTTP_404_NOT_FOUND)
