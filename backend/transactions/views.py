...
from transactions.models import Candidate, Task
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if creator.remaining_points < task.reward_points:
                return JsonResponse(
                    {"error": "Insufficient points for the transaction"}, status=400
                )
    ...
    if request.method == "POST":
        try:
            ...
            if task.task_status != "awaiting":
                return JsonResponse(
                    {"error": "Task is already assigned or not available"}, status=400
                )
            ...
            if task.creator_id.user_id == int(creator_id):
                ...
                return JsonResponse(
                    {"error": "You are not the creator of this task"}, status=403
                )
...
def apply_for_task(request):
    if request.method == "POST":
        ...
        if not hunter_id or not task_id:
            return JsonResponse(
                {"error": "Hunter ID and Task ID are required"}, status=400
            )
        ...
        try:
            ...
            if task.task_status != "awaiting":
                return JsonResponse(
                    {"error": "Task is not available for application"}, status=400
                )

            # 检查是否已申请
            if Candidate.objects.filter(task_id=task, user_id=hunter_id).exists():
                return JsonResponse(
                    {"error": "You have already applied for this task"}, status=400
                )

            candidate = Candidate.objects.create(
                task_id=task, user_id=User.objects.get(user_id=hunter_id)
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if task.assignee_id.user_id == int(hunter_id):
                ...
                return JsonResponse(
                    {"error": "You are not the assignee of this task"}, status=403
                )
        except Task.DoesNotExist:
            return JsonResponse({"error": "Task does not exist"}, status=404)
    return JsonResponse({"error": "Invalid request method"}, status=400)
...
def review_task(request):
    if request.method == "POST":
        ...
        if not creator_id or not task_id:
            return JsonResponse(
                {"error": "Creator ID and Task ID are required"}, status=400
            )
