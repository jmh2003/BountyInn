...
#                 Q(comment_nickname__icontains=search)
#                 | Q(comment_content__icontains=search)
#             )
#         if start_date:
#             try:
#                 start_datetime = parse_datetime(start_date)
...
from .models import forum
from .serializers import ForumSerializer
from mysite.open_ai import get_result_openai
...
def comments(request):
    ...
    if request.method == "GET":
        ...
        serializer = ForumSerializer(data=request.data)
        if request.data.get("comment_content"):
            question = request.data["comment_content"]
            answer = get_result_openai(question)
            print("test answer:",answer)
            if  "I CAN NOT ANSWER" in answer:
                return Response({"error": "I CAN NOT ANSWER"}, status=status.HTTP_400_BAD_REQUEST)
