...
def get_user_tasks(request):
    ...
    try:
        ...
        for task in tasks:
            # 查询尝试接取此任务的候选人
            candidates = Candidate.objects.filter(task_id=task["task_id"]).values(
                "user_id__nickname"
            )
            candidate_nicknames = [
                candidate["user_id__nickname"] for candidate in candidates
            ]
...
def submit_task_outcome(request):
    ...
    if not task_outcome or task_outcome.strip() == "":
        return Response(
            {"error": "Task outcome cannot be empty."},
    ...
    try:
        ...
        task.save()
        return Response(
            {"message": "Task outcome updated successfully"}, status=status.HTTP_200_OK
        )
    except Task.DoesNotExist:
...
def delete_task(request):
    if request.method == "PATCH":  # 修改为PATCH请求
        try:
            ...
            return JsonResponse(
                {
                    "message": "Task status updated to aborted and related candidate data deleted"
                },
                status=200,
            )
...
def submit_review(request):
    ...
    logger.debug(
        f"Received submit_review data: task_id={task_id}, reviewee_id={reviewee_id}, reviewer_id={reviewer_id}, rating={rating}, comment={comment}"
    )

    if not all([task_id, reviewee_id, reviewer_id, rating, comment.strip()]):
        return Response(
            {"error": "所有字段都是必填的。"}, status=status.HTTP_400_BAD_REQUEST
        )
